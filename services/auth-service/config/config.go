...
func Init() {
	...
	systemlogger.Systemf("✅ SISTEMA PRONTO")
	systemlogger.Systemf("🚪 Porta: %s", os.Getenv("PRODUCT_SERVICE_PORT"))
	systemlogger.Systemf("⏱️ Tempo: %v", time.Since(startTime).Round(time.Millisecond))
